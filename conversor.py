...
class WierdXMLGenerator(object):
    ...
    def inflate(self):
        ...
        root.add_field(Field('name',
            value='Plan Contable PONTECORVO'))
    ...
    def process_account_row(self, row):
        ...
        if row['entidad']:
            record.add_field(Field('party_required', {'eval': 'True'}))
...
def main(types, chart, output):
    ...
    g = WierdXMLGenerator(types, chart)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--account-types', dest='account_types',
        help="Account Type Chart CSV filename", required=True)
    parser.add_argument('--account-chart', dest='account_chart',
        help="Account Chart CSV filename", required=True)
    parser.add_argument('--output', dest='output', default='chart.xml',
        help="Account Chart XML filename")
    options = parser.parse_args()
    #sys.argv = []  # clean argv for trytond

    main(options.account_types, options.account_chart, options.output)
